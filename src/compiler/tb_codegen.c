...
static inline bool tinybackend_value_is_addr(BEValue *value) { return value->kind == BE_ADDRESS || value->kind == BE_ADDRESS_FAILABLE; }
static inline bool tinybackend_value_is_bool(BEValue *value) { return value->kind == BE_BOOLEAN; }

static TB_DataType tinybackend_get_type(Type* type) {
    switch (type->type_kind) {
        case TYPE_U8: case TYPE_I8: return TB_TYPE_I8(1);
        case TYPE_U16: case TYPE_I16: return TB_TYPE_I16(1);
        case TYPE_U32: case TYPE_I32: return TB_TYPE_I32(1);
        case TYPE_U64: case TYPE_I64: return TB_TYPE_I64(1);
        default:
        TODO
    }
}
...
TB_Register tinybackend_emit_load_aligned(GenContext *c, TB_DataType dt, TB_Register pointer, AlignSize alignment, const char *name)
{
...
void tinybackend_store_bevalue(GenContext *c, TB_DataType type, BEValue *destination, BEValue *value) { 
	...
	{
		value->value = tinybackend_emit_load_aligned(c, tinybackend_get_type(value->type), value->value, value->alignment, "");
		value->kind = BE_VALUE;
	}
	...
	switch (value->kind)
	{
		...
		FALLTHROUGH;
		case BE_VALUE:
		tb_inst_store(c->function, tinybackend_get_type(destination->type), destination->value, value->value, alignment ? alignment : type_abi_alignment(value->type));
		return;
		case BE_ADDRESS:
		{
			// Here we do an optimized(?) memcopy.
			ByteSize size = type_size(value->type);
			TB_Register copy_size = tb_inst_iconst(c->function, size <= UINT32_MAX ? TB_TYPE_I32(1) : TB_TYPE_I64(1), size);
...
static TB_Register tinybackend_emit_local_decl(GenContext *c, Decl *decl) {
	...
	if (init)
	{
		...
		if (init->expr_kind != EXPR_UNDEF)
		{
			...
			tinybackend_emit_assign_expr(c, &value, decl->var.init_expr, (TB_Register) ((uintptr_t) decl->var.failable_ref));
		}
	}
	else
	{
